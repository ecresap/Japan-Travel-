...
const travelData = {
  kyoto: {
    ...
    ,
    // List of recommended hotels in Kyoto. Each item includes a name and address.
    hotels: [
      {
        name: 'Prince Smart Inn Kyoto Sanjo',
        address: '325 Maruyacho, Nakagyo Ward, Kyoto, 604-8086, Japan'
      }
    ]
  },
  osaka: {
    ...
    ,
    // List of recommended hotels in Osaka. Each item includes a name and address.
    hotels: [
      {
        name: 'Hotel Forza Osaka Namba Dotonbori',
        address: '3F, 1-4-22, Dotonbori, Chuo-ku, Osaka 5420071 Japan'
      }
    ]
  },
  tokyo: {
    ...
    ,
    // List of recommended hotels in Tokyo. Each item includes a name and address.
    hotels: [
      {
        name: 'Miyako City Tokyo Takanawa',
        address: '3 Chome-19-17 Takanawa, Minato City, Tokyo 108-0074, Japan'
      }
    ]
  }
};
...
function showCity(cityKey) {
  ...
  if (cityKey === 'tokyo' && Object.keys(city.neighborhoods).length === 0) {
    ...
    // Insert a "Hotels" button after the map button if this city has hotels defined.
    if (Array.isArray(city.hotels) && city.hotels.length > 0) {
      const hotelsBtn = document.createElement('button');
      // Use the same full‑row style as the map button to span the width
      hotelsBtn.className = 'button hotels-button';
      hotelsBtn.textContent = 'Hotels';
      hotelsBtn.addEventListener('click', () => showHotels(cityKey));
      list.appendChild(hotelsBtn);
    }
    // Insert a "Favorites" button immediately after the hotels button (or map button if no hotels).
...
/**
 * Render a list of hotels for the selected city. Each hotel is shown with
 * its name (as a Google Maps link) and its address beneath. This view
 * appears when the "Hotels" button on a city page is clicked. It uses
 * breadcrumbs similar to other views and provides a back link to return
 * to the city menu.
 *
 * @param {string} cityKey
 */
function showHotels(cityKey) {
  const city = travelData[cityKey];
  if (!city) return;
  // Update breadcrumbs: Home / City / Hotels
  breadcrumbsEl.innerHTML = '';
  const homeLink = document.createElement('a');
  homeLink.href = '#';
  homeLink.textContent = 'Home';
  ...
  const sep2 = document.createTextNode(' / ');
  breadcrumbsEl.appendChild(sep2);
  const hotelsSpan = document.createElement('span');
  hotelsSpan.textContent = 'Hotels';
  breadcrumbsEl.appendChild(hotelsSpan);
  // Clear main content
  contentEl.innerHTML = '';
  ...
  contentEl.appendChild(back);
  // Card wrapper for hotels list
  const card = document.createElement('div');
  card.className = 'card';
  const h2 = document.createElement('h2');
  h2.textContent = `${city.name} — Hotels`;
  card.appendChild(h2);
  // List of hotels
  ...
  if (!city.hotels || city.hotels.length === 0) {
    ...
    p.textContent = 'No hotels added yet.';
    card.appendChild(p);
  } else {
    city.hotels.forEach((hotel) => {
      const li = document.createElement('li');
      li.style.marginBottom = '0.6rem';
      // Hotel name as a link to Google Maps search
      const nameDiv = document.createElement('div');
      const nameLink = document.createElement('a');
      nameLink.className = 'poi-link';
      // Use the city name in the query to improve search accuracy
      nameLink.href = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(hotel.name + ' ' + city.name)}`;
      nameLink.target = '_blank';
      nameLink.rel = 'noopener noreferrer';
      nameLink.textContent = hotel.name;
      nameDiv.appendChild(nameLink);
      li.appendChild(nameDiv);
      // Address line
      if (hotel.address) {
        const addrDiv = document.createElement('div');
        addrDiv.style.fontSize = '0.95rem';
        addrDiv.style.opacity = '0.9';
        addrDiv.textContent = hotel.address;
